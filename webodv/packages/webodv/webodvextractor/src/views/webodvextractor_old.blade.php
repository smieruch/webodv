@extends('layouts.webodv_layout')

@section('content')


    <div class="container-fluid webodvextractor_main">
	
	
	<!-- ##################### pager ######################## -->
	<!-- only show at md and above: d-none d-md-block -->
	<div class="d-none d-md-block" id="pager_large">
	    <ul class="pagination justify-content-center" style="margin:10px 0">
		<li id="pager_large_0" class="page-item webodv_pager_left" ><a class="page-link" href="#"><i class="fa fa-arrow-left"></i>&nbsp;</a></li>
		<li id="pager_large_1" class="page-item webodv_pager"><a class="page-link" href="#">1. Select stations</a></li>
		<li id="pager_large_2" class="page-item webodv_pager"><a class="page-link" href="#">2. Select variables</a></li>
		<li id="pager_large_3" class="page-item webodv_pager"><a class="page-link" href="#">3. Visualization</a></li>
		<li id="pager_large_4" class="page-item webodv_pager"><a class="page-link" href="#">4. Download</a></li>
		<li id="pager_large_5" class="page-item webodv_pager"><a class="page-link exit-pager" href="#">5. Exit</a></li>
		<li id="pager_large_6" class="page-item webodv_pager_right"><a class="page-link" href="#">&nbsp;<i class="fa fa-arrow-right"></i></a></li>
	    </ul>
	</div>
	<!-- only show below md: d-md-none -->
	<div class="d-md-none" id="pager_small" style="">
	    <ul class="pagination justify-content-center" style="margin:10px 0">
		<li id="pager_small_0" class="page-item webodv_pager_left"><a class="page-link" href="#"><i class="fa fa-arrow-left"></i>&nbsp;</a></li>
		<li id="pager_small_1" class="page-item webodv_pager"><a class="page-link" href="#">1.</a></li>
		<li id="pager_small_2" class="page-item webodv_pager"><a class="page-link" href="#">2.</a></li>
		<li id="pager_small_3" class="page-item webodv_pager"><a class="page-link" href="#">3.</a></li>
		<li id="pager_small_4" class="page-item webodv_pager"><a class="page-link" href="#">4.</a></li>
		<li id="pager_small_5" class="page-item webodv_pager"><a class="page-link exit-pager" href="#">5.</a></li>
		<li id="pager_small_6" class="page-item webodv_pager_right"><a class="page-link" href="#">&nbsp;<i class="fa fa-arrow-right"></i></a></li>
	    </ul> 	    
	</div>
	<!-- ##################### pager ######################## -->


	

	<!-- ##################### page1 ######################## -->
	<div class="webodvextractor_page webodvextractor_page_1">
	    <div class="row">
		<div class="col-md-2 text-center">
		</div>
		<div class="col-md-8 text-center">
		    Click <i>Zoom in</i> to define a sub-region, <i>Apply</i> to select the sub-region, or <i>Zoom out</i> to return to full domain. Select <i>Dates</i> in the format mm/dd/yyyy. Use the <i>Required variables</i> as a station filter. 
		    <br><br>
		</div>
		<div class="col-md-2 text-center">
		    <button class="btn btn-responsive btn-block btn-outline-success text-left download_image" id="">Download image</button>		    
		    <a class="" href="#" style="display:none;" id="download_image_a_page1" download="image.gif">
			<button id="download_image_button_page1"></button>		    
		    </a>
		</div>
	    </div>

	    <div class="row">
		<div class="col-md-3 text-left station_select_left_column order-2 order-md-1">	    
		    <h5 class="text-info">Selection status</h5>
		    <div class="border border-info rounded text-info">
			<div class="container num_stations_div" style="margin-left:0px;">
			    Stations:  &nbsp; <span class="num_stations"></span> of <span class="num_stations_total"></span>
			    <br>
			    Output variables:  &nbsp; <span class="num_variables">0</span> of <span class="num_variables_total"></span>
			</div>
		    </div>
		    <br>
		    @if ($extractor_info['station_info_show'])
			<h5 class="text-info">Station info <i id="station_info_help"  class="fa fa-question-circle-o help_color"></i></h5>
			<button class="btn btn-responsive btn-block btn-outline-info" id="station_info" style="">
			    <div class="text-left" style="float:left;">Cruise: 
			    </div>
			    <div class="text-right" style="float:right;padding-top:4px;">
				<i class="fa fa-caret-down"></i>
			    </div>
			</button>
			<div id="get_dock_metavar_labels_list" style="display:none; overflow-y: scroll; height: 300px; z-index: 1000; position: absolute; background-color: white;"></div>
			<br>
		    @endif
		    @if ($extractor_info['cruises_show'])
			<div>
			    <h4 id="Cruises">Cruises</h4>
			    <div class="selectpicker_wrapper cruises text-left">
				<select class="selectpicker form-control " id="select_cruise" data-style="btn-outline-secondary" data-live-search="true" multiple data-selected-text-format="count" data-actions-box="true">
				    @foreach ($extractor_info['cruises'] as $cruise)
					<option value="{{ $cruise }}" selected="selected" >{{ $cruise}}</option>
				    @endforeach
				</select>
			    </div>
			</div>
			<br>
		    @endif
		    @if ($extractor_info['zoom_show'])
			<div>
			    <h4>Map domain</h4>
			    <div class="row Yhide">
				<div class="d-lg-none col-md-12 mb-2">    <!-- show below lg and add margin-bottom-2-->
				    <input type="button" class="btn btn-responsive btn-block btn-outline-secondary zoom_in text-left " value="Zoom mode">
				</div>
				<div class="d-none d-lg-block col-lg-6"> <!-- show above lg with no margin -->
				    <input type="button" class="btn btn-responsive btn-block btn-outline-secondary zoom_in text-left " value="Zoom mode">
				</div>
				<div class="col-lg-6 col-md-12 mb-2"> 
				    <input type="button" class="btn btn-responsive btn-block btn-outline-secondary zoom_out text-left " value="Zoom out">
				</div>
			    </div>
			    <div class="row Yhide">
				<div class="col-md-12"> 
				    <input type="button" class="btn btn-responsive btn-block btn-outline-secondary zoom_full_range text-left " value="Full range">
				</div>
			    </div>
			</div>
			<br>
		    @endif
		    @if ($extractor_info['date_show'])
			<div>		   
			    <h4>Date <i id="date_help"  class="fa fa-question-circle-o help_color"></i></h4>
			    <div class="input-group mb-2">
				from: mm/dd/yyyy
				<input type="text" name="date1" class="form-control btn btn-responsive btn-block btn-outline-secondary text-left " id="date1">
			    </div>
			    <div class="input-group">
				to: mm/dd/yyyy
				<input type="text" name="date2" class="form-control btn btn-responsive btn-block btn-outline-secondary text-left " id="date2">
			    </div>
			</div>
			<br>
		    @endif
		    @if ($extractor_info['required_variables_show'])
			<div>
			    <h4>Required variables <i id="required_variables_help"  class="fa fa-question-circle-o help_color"></i></h4>
			    <button class="btn btn-responsive btn-block btn-outline-secondary text-left" id="required_vars_button">0 variables selected</button>
			</div>
			<br>
		    @endif
		    @if ($extractor_info['reset_show'])
			<div>
			    <h4>Reset</h4>
			    <button class="btn btn-responsive btn-block btn-outline-danger text-left" id="reset">Reset</button>
			</div>
			<br>
		    @endif
		    @if ($extractor_info['point_size_show'])
			<div>
			    <!-- <h4>Point size</h4> -->
			    <h5 class="text-secondary">Point size</h5>
			    <div class="selectpicker_wrapper">
				<select class="selectpicker form-control" id="point_size" data-style="btn btn-responsive btn-block btn-outline-secondary ">
				    <option value="0.1">0.1</option>
				    <option value="0.5">0.5</option>
				    <option value="0.7">0.7</option>
				    <option value="1">1</option>
				    <option value="1.5">1.5</option>
				    <option value="2">2</option>
				    <option value="2.5">2.5</option>
				    <option value="3">3</option>
				    <option value="3.5">3.5</option>
				    <option value="4">4</option>
				    <option value="5">5</option>
				    <option value="6">6</option>
				    <option value="7">7</option>
				    <option value="8">8</option>
				</select>
			    </div>
			</div>		    
			<br>
		    @endif			
		</div>
		
		<div class="bottom-div col-md-9 text-center swipe_on img_col order-1 order-md-2"> <!-- d-none d-md-block -->
		    <div class="wsODV_map_container"> <!-- show only above md -->		    
			<img class="img-fluid wsODV_map" id="wsODV_map" alt="" style="">
		    </div>
		</div>
	    </div>
	</div>

	<!-- required_vars modal -->
	<div class="modal fade" id="required_vars_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	    <div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
		    <div class="modal-header">
			<h4 class="modal-title" id="exampleModalLabel">Required variables</h4>
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
			    <span aria-hidden="true">&times;</span>
			</button>
		    </div>
		    <div class="modal-body">
			<div class="treeview_div" style="">
			    <div class="row">
				<div class="col-lg-8">

				    <div class="container-fluid d-lg-none">
					<div class="row">
					    <div class="col-lg-3">
					    </div>
					    <div class="col-lg-3">
						<button class="btn btn-outline-primary btn-block text-left treeview_button_apply" style="" id="treeview_button_apply" >Apply</button>
						<br>
					    </div>
					    <div class="col-lg-3">
						<button class="btn btn-outline-secondary btn-block text-left treeview_button_cancel" style="" id="treeview_button_cancel" >Cancel</button>
						<br>
					    </div>
					    <!-- <div class="col-lg-3">
						 <button class="btn btn-outline-secondary btn-block text-left treeview_button_collapse" style="" id="treeview_button_collapse" >Collapse</button>
						 <br>
						 </div> -->
					    <div class="col-lg-3">
						<button class="btn btn-outline-danger btn-block text-left treeview_button_reset" style="" id="treeview_button_reset" >Reset</button>
						<br>
					    </div>
					</div>
				    </div>
				    <!-- search -->
				    <div class="dropdown">
					<div class="input-group">
					    <input id="search_input" type="text" class="form-control" placeholder="Search" onclick="this.select()" style="">
					    <span class="input-group-append" style="border-left:0">
						<button type="submit" id="search_button">
						    <i class="fa fa-search"></i>
						</button>
					    </span>
					    <ul class="dropdown-menu h-scroll" id="search_output">
					    </ul>
					</div>
				    </div>
				</div>
				<div class="col-lg-4 d-none d-lg-block">
				    <h4>Selected variables</h4>
				</div>
				<div class="col-lg-8">
				    <br>
				    <div class="treeview_container">
					<div id="treeview_container" class="hummingbird-treeview text-left" style="overflow-y: scroll; height: 400px;">
    					    <ul id="treeview_requested_vars" class="hummingbird-base">
					    </ul>
					</div>
				    </div>
				</div>
				<div class="col-lg-4 text-left selected_vars" style="overflow-y: scroll; height:400px;">
				    <div class="d-lg-none">
					<h4>Selected variables</h4>
				    </div>
				    <h4 class="h4_selected_variables"></h4>
				</div>
			    </div>
			</div>
		    </div>
		    <div class="modal-footer">
			<div class="container-fluid d-none d-lg-block ">
			    <div class="row">
				<div class="col-lg-3">
				</div>
				<div class="col-lg-3">
				    <button class="btn btn-outline-primary btn-block text-left treeview_button_apply" style="" id="treeview_button_apply" >Apply</button>
				    <br>
				</div>
				<div class="col-lg-3">
				    <button class="btn btn-outline-secondary btn-block text-left treeview_button_cancel" style="" id="treeview_button_cancel" >Cancel</button>
				    <br>
				</div>
				<!-- <div class="col-lg-3">
				     <button class="btn btn-outline-secondary btn-block text-left treeview_button_collapse" style="" id="treeview_button_collapse" >Collapse</button>
				     <br>
				     </div> -->
				<div class="col-lg-3">
				    <button class="btn btn-outline-danger btn-block text-left treeview_button_reset" style="" id="treeview_button_reset" >Reset</button>
				    <br>
				</div>
			    </div>
			</div>
		    </div>
		</div>
	    </div>
	</div>


	<div class="modal fade" id="required_vars_help_modal" tabindex="-1" role="dialog" aria-labelledby="required_vars_helpModalLabel" aria-hidden="true">
	    <div class="modal-dialog" role="document">
		<div class="modal-content">
		    <div class="modal-header">
			<h4 class="modal-title" id="required_vars_helpModalLabel">Select required variables</h4>
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
			    <span aria-hidden="true">&times;</span>
			</button>
		    </div>
		    <div class="modal-body">
			Select one or more variables from the <i>Required
			variables</i> tree to exclude stations not
			containing data for these variables. Only
			stations containing data for all selected <i>Required variables</i> are
			accepted and shown on the map. Warning: Selecting too many
			required variables easily leads to zero matching stations.
		    </div>
		    <div class="modal-footer">
			<button class="btn btn-outline-secondary text-left" data-dismiss="modal"" style="" id="required_vars_help_close_button" >Close</button>
		    </div>
		</div>
	    </div>
	</div>
	
	
	<div class="modal fade" id="date_help_modal" tabindex="-1" role="dialog" aria-labelledby="date_helpModalLabel" aria-hidden="true">
	    <div class="modal-dialog" role="document">
		<div class="modal-content">
		    <div class="modal-header">
			<h4 class="modal-title" id="date_helpModalLabel">Select date</h4>
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
			    <span aria-hidden="true">&times;</span>
			</button>
		    </div>
		    <div class="modal-body">
			Type a date in the format mm/dd/yyyy or use
			the calender widget. Range:
			01/01/1850-12/31/2021. Click on the widget's
			top line to switch between years, click again
			to switch between decades and so on.

		    </div>
		    <div class="modal-footer">
			<button class="btn btn-outline-secondary text-left" data-dismiss="modal" style="" id="date_help_close_button" >Close</button>
		    </div>
		</div>
	    </div>
	</div>

	<div class="modal fade" id="station_info_help_modal" tabindex="-1" role="dialog" aria-labelledby="station_info_helpModalLabel" aria-hidden="true">
	    <div class="modal-dialog" role="document">
		<div class="modal-content">
		    <div class="modal-header">
			<h4 class="modal-title" id="station_info_helpModalLabel">Station info</h4>
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
			    <span aria-hidden="true">&times;</span>
			</button>
		    </div>
		    <div class="modal-body">
			Click on the map to get infos about specific stations.
		    </div>
		    <div class="modal-footer">
			<button class="btn btn-outline-secondary text-left" data-dismiss="modal" style="" id="date_help_close_button" >Close</button>
		    </div>
		</div>
	    </div>
	</div>

	<div class="modal fade" id="extractor_help_modal" tabindex="-1" role="dialog" aria-labelledby="extractor_helpModalLabel" aria-hidden="true">
	    <div class="modal-dialog" role="document">
		<div class="modal-content">
		    <div class="modal-header">
			<h4 class="modal-title" id="extractor_helpModalLabel">Help</h4>
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
			    <span aria-hidden="true">&times;</span>
			</button>
		    </div>
		    <div class="modal-body">
			<ul>
			    <li>Use the left and right arrow keys to switch between pages.</li>
			</ul>
		    </div>
		    <div class="modal-footer">
			<button class="btn btn-outline-emodnet emodnet-orange-border text-left" data-dismiss="modal"" style="" id="required_vars_help_close_button" >Close</button>
		    </div>
		</div>
	    </div>
	</div>


	<div class="modal fade" id="inactivity_modal" tabindex="-1" role="dialog" aria-labelledby="inactivityModalLabel" aria-hidden="true">
	    <div class="modal-dialog" role="document">
		<div class="modal-content">
		    <div class="modal-header">
			<h4 class="modal-title" id="inactivityLabel">Session closed due to inactivity!</h4>
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
			    <span aria-hidden="true">&times;</span>
			</button>
		    </div>
		    <div class="modal-body">
			Your session has been closed due to 1 h of inactivity. Please reload the page or leave the page via <i>Exit</i> or webODV.
		    </div>
		    <div class="modal-footer">
			<button class="btn btn-outline-emodnet emodnet-orange-border text-left" data-dismiss="modal" style="" id="date_help_close_button" >Close</button>
		    </div>
		</div>
	    </div>
	</div>

	<!-- ##################### page1 ######################## -->



	

	<!-- ##################### page2 ######################## -->
	<div class="webodvextractor_page webodvextractor_page_2" style="display:none;">
	    <div class="row">
		<div class="col-md-2 text-center">
		</div>
		<div class="col-md-8 text-center">
		    Select output variables from the <i>Output variables</i> treeview.
		    <br>
		</div>
		<div class="col-md-2 text-center">
		</div>
	    </div>
	    <br>
	    <div class="row">
		<div class="col-md-3 text-left">
		    <h5 class="text-info">Selection status</h5>
		    <div class="border border-info rounded text-info">
			<div class="container" style="margin-left:0px;">
			    Stations:  &nbsp; <span class="num_stations"></span> of <span class="num_stations_total"></span>
			    <br>			    
			    Output variables:  &nbsp; <span class="num_variables">0</span> of <span class="num_variables_total"></span>
			</div>
		    </div>
		    <br>

		    <!-- <button class="btn btn-outline-secondary btn-block text-left" style="" id="treeview_button_collapse_vars" >Collapse</button> -->
		    <!-- <h4>Reset</h4>
			 <button class="btn btn-outline-danger btn-block text-left" style="" id="treeview_button_reset_vars" >Reset</button>
			 <br> -->
		</div>
		<div class="col-md-6 text-left">
		    <h4>Output variables</h4>
		    <div class="dropdown">
			<div class="input-group">
			    <input id="search_input_vars" type="text" class="form-control" placeholder="Search" onclick="this.select()" style="">
			    <span class="input-group-append" style="border-left:0">
				<button type="submit" id="search_button_vars">
				    <i class="fa fa-search"></i>
				</button>
			    </span>
			    <ul class="dropdown-menu h-scroll" id="search_output_vars">
			    </ul>
			</div>
		    </div>
		    <br>

		    <div class="treeview_div_vars">
			<div id="treeview_container_vars" class="hummingbird-treeview text-left" style="overflow-y: scroll; height: 400px;">
    			    <ul id="treeview_vars" class="hummingbird-base">
			    </ul>
			</div>
		    </div>
		    <br>
		</div>
		<div class="col-md-3 text-left">
		    <h4>Selected variables</h4>
		    <div class="selected_variables_vars" style="overflow-y: scroll; height: 400px;">
			<h4 id="h4_selected_variables_vars"></h4>
		    </div>
		</div>
	    </div>
	</div>
	<!-- ##################### page2 ######################## -->


	<!-- ##################### page3 ######################## -->
	<div class="webodvextractor_page webodvextractor_page_3" style="display:none;">
	    <div class="row">
		<div class="col-md-2 text-center">
		</div>
		<div class="col-md-8 text-center">
		    Visualize selected data. Set <i>Axes variables</i> and <i>Axes ranges</i>.
		    <br>
		</div>
		<div class="col-md-2 text-center">
		    <button class="btn btn-responsive btn-block btn-outline-success text-left download_image" id="">Download image</button>
		    <a class="" href="#" style="display:none;" id="download_image_a_page3" download="image.gif">
			<button id="download_image_button_page3"></button>		    
		    </a>
		</div>
	    </div>
	    <br>
	    <div class="row">
		<div class="col-md-3 text-left">
		    <h5 class="text-info">Selection status</h5>
		    <div class="border border-info rounded text-info">
			<div class="container" style="margin-left:0px;">
			    Stations:  &nbsp; <span class="num_stations"></span> of <span class="num_stations_total"></span>
			    <br>
			    Output variables:  &nbsp; <span class="num_variables">0</span> of <span class="num_variables_total"></span>
			</div>
		    </div>
		    <br>
		    <div>
			<h4>
			    Axes variables
			</h4>
			<!-- <div class="text-left">X-Variable</div> -->
			<!-- <button class="btn btn-responsive btn-block btn-outline-emodnet emodnet-orange-border text-center" id="set_x_var">X-Variable</button> -->
			<div class="row bottom-three flex-nowrap">
			    <div class="col-sm-2" style="padding-top:6px; padding-left:25px;">
				X:
			    </div>
			    <div class="col-sm-10">
				<div class="selectpicker_wrapper text-left">
				    <select class="selectpicker form-control text-left" id="set_x_var" data-style="btn text-left btn-responsive btn-block btn-outline-secondary" data-live-search="true" data-size="4">
				    </select>
				</div>
			    </div>
			</div>
			<br>
			<!-- <div class="text-left">Y-Variable</div> -->
			<!-- <button class="btn btn-responsive btn-block btn-outline-emodnet emodnet-orange-border text-center" id="set_y_var">Y-Variable</button> -->
			<div class="row  flex-nowrap">
			    <div class="col-sm-2 " style="padding-top:6px; padding-left:25px;">
				Y:
			    </div>
			    <div class="col-sm-10">
				<div class="selectpicker_wrapper">
				    <select class="selectpicker form-control" id="set_y_var" data-style="btn btn-responsive btn-block btn-outline-secondary" data-live-search="true" data-size="4">
				    </select>
				</div>
			    </div>
			</div>		
			<br>
			<!-- <div class="text-left">X-Range </div> -->
			<h4>
			    Axes ranges <i class="fa fa-question-circle-o help_color" style="" data-toggle="tooltip" data-placement="auto" title="Set range with maximum three decimal places."></i>
			</h4>
			<div class="row  flex-nowrap">
			    <div class="col-sm-2" style="padding-top:30px; padding-left:25px;">
				X:
			    </div>
			    <div class="col-md-5 mb-2 text-center">
				minimum:
				<input type="text" name="xx" class="form-control btn btn-responsive btn-block btn-outline-secondary text-left range" id="x_range_from" value="0">
				<!-- data-toggle="tooltip" data-placement="auto" title="Set range with maximum three decimal places." -->
			    </div>
			    <div class="col-md-5 mb-2 text-center">
				maximum:
				<input type="text" name="xx" class="form-control btn btn-responsive btn-block btn-outline-secondary text-left range" id="x_range_to" value="20">				
			    </div>			    
			</div>
			<div class="row  flex-nowrap">
			    <div class="col-sm-2" style="padding-top:30px; padding-left:25px;">
				Y:
			    </div>
			    <div class="col-md-5 mb-2 text-center order-1" id="y_minimum">
				<span id="y_minimum_text">minimum:</span>
				<input type="text" name="xx" class="form-control btn btn-responsive btn-block btn-outline-secondary text-left range" id="y_range_from" value="0">
			    </div>
			    <div class="col-md-5 mb-2 text-center order-2" id="y_maximum">
				<span id="y_maximum_text">maximum:</span>
				<input type="text" name="xx" class="form-control btn btn-responsive btn-block btn-outline-secondary text-left range" id="y_range_to" value="150">
			    </div>
			</div>			
		    </div>
		    <br>
		    @if ($extractor_info['outliers_show'])
		    <div>
			<h4>Outliers <i class="fa fa-question-circle-o help_color" style="" data-toggle="tooltip" data-placement="auto" title="Check to hide outliers."></i></h4>
			<div class="border border-secondary rounded text-left" style="padding-top:8px;padding-bottom:8px;">
			    <div class="form-check">
				<input class="form-check-input" type="checkbox" value="" id="outliers">
				<label class="form-check-label" for="outliers">
				    Hide Outliers
				</label>
			    </div>
			</div>			    
			<!-- <button class="btn btn-responsive btn-block btn-outline-emodnet emodnet-orange-border text-center" id="outliers">Outliers</button> -->
		    </div>
		    <br>
		    @endif
		    <div>
			<h4>
			    Full range <i class="fa fa-question-circle-o help_color" style="" data-toggle="tooltip" data-placement="auto" title="Set X and Y axes of the selected variables to cover the full data range."></i>
			</h4>
			<button class="btn btn-responsive btn-block btn-outline-danger text-left" id="reset_p3">Full range</button>
		    </div>
		    <br>
		    <!-- endif -->
		</div>
		<div class="bottom-div col-md-8 col-lg-9 text-center swipe_on img_col order-1 order-md-2"> 
		    <div class="wsODV_map_container"> 
			<img class="img-fluid wsODV_map" id="wsODV_map_viz" alt="" style="">
		    </div>
		</div>
	    </div>
	</div>
	<!-- ##################### page3 ######################## -->



	<!-- ##################### page4 ######################## -->
	<div class="webodvextractor_page webodvextractor_page_4" style="display:none;">
	    <div class="row">
		<div class="col-md-2 text-center">
		</div>
		<div class="col-md-8 text-center">
		    Download data in different formats. You will be notified by email when the .zip file containing the data in the format of your choice is ready.
		    <br>
		</div>
		<div class="col-md-2 text-center">
		</div>
	    </div>
	    <br>
	    <div class="row">
		<div class="col-md-3 text-left">
		    <h5 class="text-info">Selection status</h5>
		    <div class="border border-info rounded text-info">
			<div class="container" style="margin-left:0px;">
			    Stations:  &nbsp; <span class="num_stations"></span> of <span class="num_stations_total"></span>
			    <br>			    
			    Output variables:  &nbsp; <span class="num_variables">0</span> of <span class="num_variables_total"></span>
			</div>
		    </div>		    
		</div>
		<div class="col-md-1 text-left">
		</div>
		<br>
		<div class="col-md-4 text-left">
		    <h4>Download</h4>
		    <div class="form-group" id="download_group">
			<a id="geotr_download" target="_blank" download>
			    <input type="button" style="display: none;" class="btn" id="start_download">
			</a> 
			<input type="button" class="btn btn-responsive btn-block btn-outline-secondary myDownloadButton text-left" id="sel_txt" value="Spreadsheet">		    
		    </div>
		    <div class="form-group">
			<a download>
			    <input type="button" style="display: none;" class="btn" id="selodv">
			</a> 
			<input type="button" class="btn btn-responsive btn-block btn-outline-secondary myDownloadButton text-left" id="sel_odv" value="ODV Collection">
		    </div>
		    <div class="form-group">
			<a download>
			    <input type="button" style="display: none;" class="btn" id="selnc">
			</a> 
			<input type="button" class="btn btn-responsive btn-block btn-outline-secondary myDownloadButton text-left" id="sel_nc" value="netCDF">
		    </div>
		    <!-- <div class="form-group">
			 <a download>
			 <input type="button" style="display: none;" class="btn" id="selcsv">
			 </a> 
			 <input type="button" class="btn btn-responsive btn-block btn-outline-secondary myDownloadButton text-left"  id="sel_csv" value="WHP Exchange">
			 </div> -->
		</div>
		<div class="col-md-4 text-left">
		</div>
		
		<div class="modal fade" id="download_help_modal" tabindex="-1" role="dialog" aria-labelledby="download_helpModalLabel" aria-hidden="true">
		    <div class="modal-dialog" role="document">
			<div class="modal-content">
			    <div class="modal-header">
				<h4 class="modal-title" id="download_helpModalLabel">Download</h4>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
				    <span aria-hidden="true">&times;</span>
				</button>
			    </div>
			    <div class="modal-body">

			    </div>
			    <div class="modal-footer">
				<button class="btn btn-outline-secondary text-left" data-dismiss="modal" style="" id="download_help_close_button" >Close</button>
			    </div>
			</div>
		    </div>
		</div>

		<div class="modal fade " id="geotraces_download_help_modal" tabindex="-1" role="dialog" aria-labelledby="geotraces_download_helpModalLabel" aria-hidden="true">
		    <div class="modal-dialog modal-xl" role="document">
			<div class="modal-content">
			    <div class="modal-header">
				<h4 class="modal-title" id="geotraces_download_helpModalLabel">Download</h4>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
				    <span aria-hidden="true">&times;</span>
				</button>
			    </div>
			    <div class="modal-body">
				<form id="data_usage_form" style="" action="">
				    <h4>Please describe intended data usage</h4>
				    <!-- <div class="input-group"> -->

				    <div class="input-group">
					<div class="input-group-btn">
					    <div class="selectpicker_wrapper data_usage">
						<select class="selectpicker form-control " id="select_data_usage" data-style="btn-outline-secondary btn-block btn btn-responsive"  multiple data-selected-text-format="count">
						    <option value="">Compare with my data</option>
						    <option value="">Create compilation with other data for the same element or isotope</option>
						    <option value="">Do cross correlations among different elements or isotopes</option>
						    <option value="">Modelling</option>
						    <option value="">Use of data for biological studies</option>
						    <option value="">Use of data for paleoproxy research</option>
						    <option data-divider="true"></option>
						    <option  disabled value="">Other? Please specify</option>				    
						</select>
					    </div>
					</div>
					<input type="text" id="data_usage_input" class="form-control" aria-label="Data usage" placeholder="Specify data usage or select from dropdown">
				    </div>

				    
				    <!-- </div> -->
				    <br>
				    <h4>Please describe your funding source</h4>
				    <input  class="form-control" type="text" value="" name="funding_source" id="funding_source" placeholder="Funding source" required autofocus>
				    <br>
				    <h4>Please accept the Data Usage Agreement</h4>
				    <div class="jumbotron" style="padding: 5px">
    					@php
					require_once public_path() . '/GEOTRACES_usage_agreement_2017.txt';
					@endphp
				    </div>
				    <div class="text-center" style="">
					<h4>
					    <label class="" style="">
						<input id="data_agree" name="data_agree"  type="checkbox" value="1" required>
						<span style="">I Agree</span>
						<br>
						<div id="data_usage_form_error" style="color:a94442;"></div>
					    </label>
					</h4>
				    </div>
				</form>
				
			    </div>
			    <div class="modal-footer">
				<button class="btn btn-outline-success text-left"  style="" id="geotraces_start_download" >Start download</button>
				<button class="btn btn-outline-secondary text-left" data-dismiss="modal" style="" id="geotraces_download_help_close_button" >Cancel</button>
			    </div>
			</div>
		    </div>
		</div>


	    </div>
	</div>
	<!-- ##################### page3 ######################## -->

	<!-- ##################### page4 ######################## -->
	<div class="webodvextractor_page webodvextractor_page_5 swipe_on" style="display:none;">
	    <div class="row">
		<div class="col-md-2 text-center">
		</div>
		<div class="col-md-8 text-center">
		    Leave the <i>Data Extractor</i>. The current selections will be saved and restored once you login again.
		    <br>
		</div>
		<div class="col-md-2 text-center">
		</div>
	    </div>
	    <br>
	    <div class="row">
		<div class="col-md-2 text-center">
		</div>
		<div class="col-md-8 text-center">
		    <a href="{{ $exit_url }}">
		    <button class="btn btn-responsive btn-block btn-outline-success" style="height:100px;font-size:400%" id="exit_button">Exit</button>
		    </a>
		</div>
		<div class="col-md-2 text-center">
		</div>
	    </div>
	</div>
	<!-- ##################### page4 ######################## -->


    </div>
@endsection


